...
void updateInputCommands(uint32_t nowMs, uint32_t nowUs) {
  ...
  if (rcValid) {
    ...
    uint16_t steerPulseUs = snapshot.steerPulseUs;
    const int16_t steerCenterErrorUs =
        static_cast<int16_t>(steerPulseUs) -
        static_cast<int16_t>(BroonT870Controller::kRcSteerCalibration.centerUs);
    if (abs(steerCenterErrorUs) <=
        static_cast<int16_t>(BroonT870Controller::kRcSteerDeadbandUs)) {
      steerPulseUs = BroonT870Controller::kRcSteerCalibration.centerUs;
    }
    rcCommand.steerTargetAdc = BroonT870::rcSteerToTargetAdc(
        steerPulseUs,                                          // 중앙 데드밴드를 적용한 조향 펄스폭(μs)
...
void updateDrive(uint32_t nowMs) {
  ...
  const int16_t requestedDrivePwm = BROON_ENABLE_DRIVE_OUTPUTS
