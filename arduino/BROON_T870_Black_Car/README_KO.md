...
ROS `SerialFeedBack`의 `EStop`은 A1 throttle-cut 또는 RC 스로틀 신호 손실로
...
| A3 | 조향 포텐시오미터 | `kSteeringSensorPin` | 0~5 V 범위의 아날로그 입력만 허용 |
...
`우 180 = -1000`으로 나눠 정규화한다. 목표와 현재 위치 차이가 약 52 ADC면
좌우 모두 PWM 130, 약 105 ADC 이상이면 좌우 모두 PWM 200을 요청한다.
현재 설정은 데드밴드 8 ADC, 최소 PWM 60, 최대 PWM 200이다. 안전 끝에서
...
따라서 목표에서 멀 때의 PWM 60~200 반응을 높이면서 목표 근처의 반복 방향
반전을 억제한다.
...
PWM이 `-200` 또는 `+200`인데도 ADC가 천천히 변하면 남은 병목은 RC 매핑이
...
PWM을 만든다. 초기 제한은 목표 3 km/h, PWM 80, 목표 램프 1 km/h/s이며,
Kp 20, Ki 8, 속도 데드밴드 0.12 km/h다. 적분 포화 방지를 적용하고 목표 0,
brake, ROS timeout, RC 원격정지, RC 모드 전환 시 PI 상태를 초기화한다. RC
모드는 엔코더 폐루프가 아니라 기존 스로틀-to-PWM 방식이다.

ROS 출력이 실제 허가된 상태에서 PWM 60 이상인데 1.5초 동안 전륜 엔코더
변화가 없으면 fault 5(`FAULT_DRIVE_NO_FEEDBACK`), 측정속도가 4 km/h를 넘으면
fault 6(`FAULT_DRIVE_OVERSPEED`)을 래치하고 출력을 차단한다. 전륜 센서 하나만
있으므로 후륜 개별 속도나 전·후륜 속도 차이는 검출할 수 없다.
