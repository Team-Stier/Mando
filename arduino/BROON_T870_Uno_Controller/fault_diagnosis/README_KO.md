...
급변 250 ADC/0.25초 규칙은 2026-08-29 정상 조향 로그를 반영하여, 현재 샘플 또는
직전 샘플에 유효한 PWM이 있었고 실제 ADC가 직전 목표 오차를 줄이는 방향으로
움직인 경우를 정상 도달 응답으로 제외한다. 따라서 모터가 목표에 도달해 현재 PWM이
0으로 바뀐 캡처 경계에서도 직전 구동의 정상 이동을 센서 급변으로 오인하지 않는다.
직전 PWM이 없거나 ADC가 목표 반대방향으로 급변하면 기존처럼 이벤트를 만든다. 센서 고착은 1.5초
...
합성 로그로 정상, 정지상태 조향 오탐 방지, PWM 종료 직후 목표 도달 오탐 방지,
반대방향 센서 급변 유지, 조향 무응답·역응답·급변·안전범위 이탈·고착·헌팅,
구동 무응답·과속, CAN 오류, RC 채널 무효 및 보고서 생성을 검사한다.
