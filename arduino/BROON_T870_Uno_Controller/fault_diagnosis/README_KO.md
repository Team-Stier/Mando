...
| `STEERING_SAFE_RANGE` | 실차 교정 안전범위 180~1020을 이탈 |
| `STEERING_SENSOR_JUMP` | 짧은 시간에 비정상적으로 큰 ADC 변화 발생 |
| `STEERING_TRACKING_ERROR` | PWM 출력 중 목표오차가 지속됨 |
| `STEERING_NO_RESPONSE` | 목표가 안정된 상태에서 PWM 대비 ADC 변화가 없음 |
| `STEERING_REVERSE_RESPONSE` | 실제 ADC가 목표와 반대방향으로 계속 이동 |
| `STEERING_SENSOR_STUCK` | 목표와 PWM은 크게 변하지만 실제 ADC 범위가 고정됨 |
...
급변 250 ADC/0.25초, 반대방향 응답 0.6초, 센서 고착 1.5초 기준은 아직 실차 정상
조향 로그로 확정된 값이 아니다. 첫 RC·ROS 정상 주행에서 중앙 유지, 좌·우 step,
복귀 구간을 확보한 뒤 정상 최대 변화율보다 충분히 큰 값으로 다시 정한다.
