...
`BuildOptions.h`의 다음 두 잠금은 현재 실차 시험 승인값인 `1`이다.

- `BROON_ENABLE_ACTUATOR_OUTPUTS=1`: 안전 상태가 ARMED일 때 모터 PWM 출력을 허용한다.
- `BROON_STEERING_CALIBRATION_CONFIRMED=1`: 현재 조향 교정값 사용을 승인한다.
...
한 바퀴 굴린 실제 이동거리로 최종 보정해야 한다. 2026-08-29 RC/CAN 조향 시험 후
좌 기계 끝 1020, 좌 안전 명령 끝 1000, 직진 592, 우 안전 명령 끝 200,
우 기계 끝 180으로 보정했다. 기계 끝과 명령 끝 사이에 약 20 ADC의 소프트웨어
여유를 둔다. 포텐시오미터 체결을 건드린 경우 [교정 기록](CALIBRATION_RECORD_KO.md)을
다시 확인하고 바퀴를 띄운 상태에서 첫 좌·우 끝점 시험을 진행한다.

## 정규화 조향 제어와 ROS 속도 PI 제어

조향은 안전 명령범위를 `좌 1000 = +1000`, `직진 592 = 0`,
`우 200 = -1000`으로 나눠 정규화한다. 목표와 현재 위치 차이가 약 20 ADC면
...
방향으로만 최대 PWM 60을 허용한다. RC 조향은 `995 → 1000`, `1450 → 592`,
`1985 → 200`으로 중앙을 기준으로 나눠 보간하므로 실제 직진값을
...
않으면 fault 3(`FAULT_STEERING_STALL`)을 검출한다. 현재 `kSteeringMaxDriveMs=0`이므로
ADC가 정상적으로 움직이는 동안에는 조향 지속시간만으로 fault 4가 발생하지 않는다.
상태 로그의 `steer=target/adc/pwm`은 목표 ADC, 현재 ADC, 실제 허가 PWM을 표시한다.
무인 대회용 기본값은 `BROON_ENABLE_LATCHED_FAULTS=0`이다. fault 발생 즉시 모든
PWM을 0으로 만들고 `state=1`로 전환하며, fault를 CAN/시리얼에 남기기 위해 1초간
유지한다. 이후 실제 조건이 사라지고 스로틀 중립이 0.5초 유지되면 자동으로
`state=2`로 복귀한다. 고장 조건이 계속되면 다시 fault가 검출되어 출력은 계속
차단된다. 영구 `state=3`이 필요한 시험에서는 해당 옵션을 `1`로 변경한다.
