...
For firmware >= 7 NAV-PVT processing, `use_ros_time` (default `false`) selects
PC ROS receipt time for NavSatFix and velocity headers when enabled. With
`use_ros_time: false`, valid GNSS UTC supplies both headers. Set
`require_valid_utc: true` to reject fix and velocity publication when UTC is
invalid instead of silently switching their headers to the PC clock. The
default `require_valid_utc: false` preserves that legacy fallback. Explicit
`use_ros_time: true` always selects receipt time. Restart the driver after
changing either parameter. No clock offset is added to a measurement stamp.

UTC validation checks real Gregorian dates, clock fields, signed `nano` in
the protocol range `[-1000000000, 1000000000]`, and the ROS timestamp range.
It preserves the driver's existing flag policy: `VALID_DATE`, `VALID_TIME`,
`VALID_FULLY_RESOLVED`, and `FLAGS2_CONFIRMED_AVAILABLE` must be set. A negative
`nano` borrows from the preceding second; `nano=1000000000` carries into the
next second. The protocol's `sec=60` is normalized to the following POSIX
second and is marked separately in diagnostics. ROS time cannot represent
a leap second as a distinct civil-time label.

Private topic `~timing` (`diagnostic_msgs/DiagnosticArray`) is published once
per NAV-PVT callback, including callbacks that reject fix/velocity. Its array
header records the callback's ROS receipt time. Status
`ublox_gps: measurement timing` exposes integer-nanosecond strings:

* `gnss_utc_ns`: original UTC converted to Unix nanoseconds when the calendar
  and timestamp range are valid; an empty string otherwise. Check `utc_valid`
  before treating this candidate as a valid measurement time.
* `callback_ros_receipt_ns`, `chosen_header_stamp_ns` (empty when rejected),
  `stamp_source`, `utc_valid`, `utc_fields_valid`, `utc_invalid_reason`, and
  `fix_velocity_published` distinguish selection from rejection.
* `stamp_source` is `gnss_utc`, `ros_receipt_explicit`,
  `ros_receipt_invalid_utc_fallback`, or `invalid_utc_rejected`.
* `receipt_minus_utc_clock_offset_plus_transport_ns` is signed receipt minus
  UTC, not a pure transport latency. GNSS/PC clock offset and transport are
  mixed until their time bases are synchronized. There is no absolute-offset
  correction in this driver.
* `utc_tacc_ns` is the receiver's UTC accuracy estimate; it does not measure
  PC synchronization accuracy. `iTOW_ms`, raw validity/confirmation flags,
  original UTC date/time/nano, leap-second normalization, and selected modes
  are also reported.

Original NAV-PVT UTC, iTOW, and payload are unchanged. A valid GNSS timestamp
alone does not establish synchronization with PC-stamped IMU or encoder data;
verify the PC clock and per-sensor timing before measurement-time fusion.
